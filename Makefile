...
THREE_NODE_CACHE_WARM_KUSTOMIZE ?= deploy/kustomize/overlays/gcp-three-node/cache-warm
...
.PHONY: gcp-grafana-admin-secret gcp-alertmanager-slack-secret gcp-observability-render gcp-observability-install gcp-observability-apply gcp-observability-status gcp-observability-grafana-port-forward grafana prefect gcp-prefect-ui gcp-alert-lifecycle-test gcp-alert-lifecycle-cleanup gcp-release-render gcp-release-apply gcp-release-status gcp-release-prefect-bootstrap gcp-release-e5-download gcp-release-e5-smoke gcp-release-ingest-run gcp-release-ingestion-failure-test gcp-three-node-render gcp-three-node-cache-warm-render gcp-three-node-cache-warm-apply gcp-three-node-cache-warm-status gcp-three-node-postgresql-expand gcp-three-node-postgresql-final gcp-three-node-apps-apply gcp-three-node-observability-migrate k6-load k6-rate-limit
...
gcp-observability-grafana-port-forward: grafana

# Short aliases: auto-start the IAP kubectl tunnel when needed, then port-forward.
# Usage: `make grafana` or `make prefect` (keep the terminal open).
grafana:
	KUBECONFIG=$(GCP_KUBECONFIG) ./scripts/gcp-ui-forward.sh grafana

prefect:
	KUBECONFIG=$(GCP_KUBECONFIG) ./scripts/gcp-ui-forward.sh prefect

gcp-prefect-ui: prefect
...
gcp-three-node-cache-warm-render:
	kubectl kustomize $(THREE_NODE_CACHE_WARM_KUSTOMIZE)

gcp-three-node-cache-warm-apply:
	KUBECONFIG=$(GCP_KUBECONFIG) kubectl apply -k $(THREE_NODE_CACHE_WARM_KUSTOMIZE)

gcp-three-node-cache-warm-status:
	KUBECONFIG=$(GCP_KUBECONFIG) kubectl -n rag wait --for=condition=complete job/kuberag-llm-model-warm --timeout=30m
	KUBECONFIG=$(GCP_KUBECONFIG) kubectl -n rag wait --for=condition=complete job/kuberag-rag-api-embedding-warm --timeout=30m
	KUBECONFIG=$(GCP_KUBECONFIG) kubectl -n prefect wait --for=condition=complete job/kuberag-prefect-embedding-warm --timeout=30m
