...
- GCP observability: Prometheus, Grafana, Loki, Tempo, Pyroscope, and
  ...
  `observability`. The API has verified metrics/log/trace/profile output; the
  Prefect worker has been rolled out with OTLP configuration for its next flow.
...
Status: Core single-node observability is deployed and runtime-checked on GCP.
Alerting, k6, and the final evidence walkthrough remain pending.
...
- Provisioned Prometheus, Loki, Tempo, and Pyroscope Grafana data sources and
  a Git-managed `KubeRAG Overview` dashboard.
- Verified a FastAPI health request (`trace_id` generated by the API) entered
  Loki successfully; verified Prometheus has API samples; verified Tempo has
  the complete RAG span tree; verified Pyroscope render returned profile data.
- Rebuilt/imported `kuberag-ingestion:local` and rolled out `prefect-worker`
  with OTLP configuration. It does not trigger a crawl; its telemetry will be
  verified on the next scheduled or explicitly approved flow run.

Still required for the roadmap acceptance set:

- Capture command/browser evidence under `docs/evidence/OBS-*`, including a
  Grafana dashboard and the first telemetry-bearing Prefect flow.
- Add Grafana alert rules, a Secret-backed contact point, and alert lifecycle
  evidence.
- Add the required k6 scenarios and correlate them with the dashboard.
...
- Grafana alert rules/contact point and alert lifecycle test; screenshot/command
  evidence for the deployed metrics, logs, traces and profiles.
...
- Add k6 load/rate-limit scenarios and evidence now that the browser/API route
  is stable.
- Capture a Prefect flow in the deployed Grafana data sources, then provision
  alerts before the k6 test window.
